...
fn main() -> Result<()> {
    ...
    for source_url in source_urls {
        println!("Fetching {} metadata...", translation_upper);
        let (_, translation_metadata) =
            fetch_bible_chapter(&format!("{}/{}/GEN.1", source_url, translation_upper))?;
        ...
        metadata.insert("language", translation_metadata.language.name_local);
